...
You don't have access to external documents now.
Provide the best posAsible answer based on your general knowledge.
If you do not know the answer, say "I'm sorry, I don't know the answer."
"""
)
# meta-llama/Llama-3.1-8B-Instruct
...
def get_llm():
    return HuggingFaceEndpoint(
        endpoint_url="https://api-inference.huggingface.co/models/meta-llama/Llama-3.1-8B-Instruct",  # Update if using another endpoint
        huggingfacehub_api_token=os.environ["HUGGINGFACEHUB_API_TOKEN"],
        temperature= 0.7,
        max_new_tokens= 512,
    )
...
def main():
    ...
    with st.sidebar:
        ...
        This chatbot uses:
        - ✅ FAISS vector search
        - ✅ Falcon LLM via HuggingFace Inference Endpoint
        - ✅ Smart fallback answering
        - ✅ Dictionary & Calculator fallback
        """)
        st.markdown("---")
    ...
    if user_query:
        ...
        if "define" in query_lower:
            ...
            fallback_phrases = ["i couldn't find the answer", "i could not find the answer","I can't help with that"]
            if rag_answer.lower() in fallback_phrases:
                fallback_chain = LLMChain(llm=get_llm(), prompt=fallback_prompt)
                fallback_answer = fallback_chain.run(question=user_query).strip()
